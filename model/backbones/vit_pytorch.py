...
class Attention_cross(nn.Module):
    ...
    def atten_feature(self, q, k ,v, B, N, C):
    ...
    def forward(self, x):
        ...
        qkvs = self.qkv(xs).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        qs, ks, vs = qkvs[0], qkvs[1], qkvs[2]
        qkvt = self.qkv(xt).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        qt, kt, vt = qkvt[0], qkvt[1], qkvt[2]
        xs = self.atten_feature(qs, ks, vs, B, N, C)
        xt = self.atten_feature(qs, kt, vt, B, N, C)
        x = (xs, xt)
        return x
...
class Block_cross(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # self.attn = Attention(
        #     dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
        self.attn = Attention_cross(dim, num_heads, qkv_bias, qk_scale, attn_drop, drop)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
    ...
    def forward(self, x):
        ...
        xs = x[0] + self.drop_path(x[0])
        xt = x[1] + self.drop_path(x[1])

        xs = xs + self.drop_path(self.mlp(self.norm2(xs)))
        xt = xt + self.drop_path(self.mlp(self.norm2(xt)))
...
class Transcross(nn.Module):
    ...
    def forward_features(self, x):
        ...
        if self.local_feature:
            ...
            x = (self.norm(x[0])[:, 0],self.norm(x[1])[:, 0])

            return x

    def forward(self, x):

        # xs = self.forward_features(x[0])
        # xt = self.forward_features(x[1])
...
def vit_base_patch16_224_Transcross(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0,
                                   drop_path_rate=0.1, local_feature=False, **kwargs):
    model = Transcross(
        img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
        qkv_bias=True, \
